...
public class RuleIT extends BaseITCase {
	...
	@Test
	public void testPeopleFlowResult() {
		SelectionCriteria criteria =
				SelectionCriteria.createCriteria(null, selectOleContext(), selectPolicyCirculationAgenda());
		...
		String peopleFlowsSelected = (String)results.getAttribute("peopleFlowsSelected");
		assertNotNull(peopleFlowsSelected);
		// the string comes back as a comma-separated list, check that we only have one
		assertEquals(1, peopleFlowsSelected.split(",").length);
	}
