...
public class DataDictionaryTypeServiceBase implements KimTypeService {
	...
	protected List<RemotableAttributeError> validateReferencesExistAndActive( KimType kimType, Map<String, String> attributes, List<RemotableAttributeError> previousValidationErrors) {
		...
		for (Map.Entry<String, BusinessObject> entry : componentClassInstances.entrySet()) {
			BusinessObjectEntry boEntry = getDataDictionaryService().getDataDictionary().getBusinessObjectEntry(entry.getKey());
			List<RelationshipDefinition> relationships = null;
			if (boEntry != null) {
				relationships = boEntry.getRelationships();
			} else {
				DataObjectEntry doEntry = getDataDictionaryService().getDataDictionary().getDataObjectEntry(entry.getKey());
				if (doEntry != null) {
					relationships = doEntry.getRelationships();
				} else {
            		throw new IllegalArgumentException("Failed to locate a business or data object entry in the data dictionary for " + entry.getKey());
            	}
			}
	...
	protected KimAttributeField getDataDictionaryAttributeDefinition( String namespaceCode, String kimTypeId, KimTypeAttribute typeAttribute, List<String> uniqueAttributes) {
		...
		try {
            if (StringUtils.isNotBlank(componentClassName)) {
                ...
                BusinessObjectEntry entry = getDataDictionaryService().getDataDictionary().getBusinessObjectEntry(componentClassName);
                if (entry != null) {
                	baseDefinition = entry.getAttributeDefinition(attributeName);
                } else {
                	DataObjectEntry dataObjectEntry = getDataDictionaryService().getDataDictionary().getDataObjectEntry(componentClassName);
                	if (dataObjectEntry != null) {
                		baseDefinition = dataObjectEntry.getAttributeDefinition(attributeName);
                	} else {
                		throw new IllegalArgumentException("Failed to locate a business or data object entry in the data dictionary for " + componentClassName);
                	}
                }
            } else {
                baseDefinition = null;
	...
	protected KimAttributeField getNonDataDictionaryAttributeDefinition(String namespaceCode, String kimTypeId, KimTypeAttribute typeAttribute, List<String> uniqueAttributes) {
		...
		/**
		 * Patched by Eric Westfall to fix bug for training purposes. Reported this bug in KULRICE-9143
		 */
